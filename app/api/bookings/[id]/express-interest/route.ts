...
async function requireStaff(request: NextRequest): Promise<{ uid: string; name: string }> {
  ...
  if (role !== "staff") {
    throw new Error("Only developer accounts can express interest")
  }
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    const { uid: staffId, name: staffName } = await requireStaff(request)
